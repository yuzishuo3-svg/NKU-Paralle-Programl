...
// MPI 版本编译指令：
// mpic++ -O2 -std=c++11 main_mpi.cpp train.cpp guessing.cpp md5.cpp -o main
...
// 所有 MPI 进程共同处理一批 guesses。
// rank 0 拥有完整 guesses，其它 rank 通过 MPI_Bcast 接收。
// 每个 rank 只处理自己负责的区间。
// 返回值：rank 0 得到本批 hash 的并行墙钟时间，其它 rank 返回 0。
