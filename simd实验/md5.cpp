...
#define G_SIMD(x, y, z) _mm_or_si128(_mm_and_si128((z), (x)), _mm_andnot_si128((z), (y)))
...
void MD5Hash_SIMD(string input[4], bit32 state_batch[4][4])
{
    ...
    for (int block = 0; block < n_blocks; block += 1)
    {
        ...
        uint32_t* ptr0 = (uint32_t*)(paddedMessage[0] + block * 64);
        uint32_t* ptr1 = (uint32_t*)(paddedMessage[1] + block * 64);
        uint32_t* ptr2 = (uint32_t*)(paddedMessage[2] + block * 64);
        uint32_t* ptr3 = (uint32_t*)(paddedMessage[3] + block * 64);

        for (int i1 = 0; i1 < 16; ++i1)
        {
            //顺序是从高到低反着写
            x[i1] = _mm_set_epi32(ptr3[i1], ptr2[i1], ptr1[i1], ptr0[i1]);
        }
    ...
    _mm_storeu_si128((__m128i*)temp_state0, state0);
    _mm_storeu_si128((__m128i*)temp_state1, state1);
    _mm_storeu_si128((__m128i*)temp_state2, state2);
    _mm_storeu_si128((__m128i*)temp_state3, state3);
